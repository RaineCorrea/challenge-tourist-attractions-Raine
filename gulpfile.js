...
function styles() {
    return src(paths.styles.main).pipe(sass({ outputStyle: "compressed" }).on("error", sass.logError)).pipe(dest("dist")).pipe(connect.reload());
}

function img(){
    return src(paths.img.all).pipe(dest("dist")).pipe(connect.reload());
}
